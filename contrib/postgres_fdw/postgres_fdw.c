						...
						FinalPathExtraData *extra)
{
	 ...
	 * unrelated query against the same server opened a connection in
	 * between, with nothing about the query itself having changed.  This is
	 * accepted as the price of not opening a connection at plan time, but it
	 * means EXPLAIN output for this query can differ from one planning to
